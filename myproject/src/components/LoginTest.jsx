...
const testLogin = async () => { setLoading(true); setResult('Testing...');
try {
...
const testFetch = async () => { setLoading(true); setResult('Testing direct fetch...');
try {
    const response = await fetch('http://localhost:5003/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: 'admin@soundsaccessories.com', password: 'admin123456' })
});
